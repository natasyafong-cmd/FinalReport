...
$total_row     = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as c FROM tasks WHERE helper_id='$user_id'"));
$active_row    = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as c FROM tasks WHERE helper_id='$user_id' AND status IN ('Accepted','In Progress')"));
$done_row      = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as c FROM tasks WHERE helper_id='$user_id' AND status='Completed'"));

$total_tasks  = $total_row['c'];
$active_tasks = $active_row['c'];
$done_tasks   = $done_row['c'];

// 3 most recent accepted tasks
$recent_result = mysqli_query($conn, "SELECT * FROM tasks WHERE helper_id='$user_id' ORDER BY id DESC LIMIT 3");
...
function status_badge($status) {
    ...
    $s = $map[$status] ?? ['b-open', htmlspecialchars($status)];
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .ep-badge { padding: 4px 12px; border-radius: 999px; font-size: 0.7rem; font-weight: 800; }
  ...
  <div class="ep-content">
    ...
    <div class="ep-welcome-card">
      ...
      <div class="ep-welcome-text">
        ...
        <span class="ep-role-pill">🦸 Helper Account</span>
      </div>
    </div>
    ...
    <div class="ep-stats-grid">
      <div class="ep-stat-card">
        ...
        <div class="ep-stat-label">Total Accepted</div>
      </div>
      <div class="ep-stat-card">
        <div class="ep-stat-icon">⏳</div>
        <div class="ep-stat-num"><?php echo $active_tasks; ?></div>
    ...
    <div class="ep-actions-grid">
      <a href="browse_tasks.php"      class="ep-action-btn ep-btn-primary">🛍 Browse new tasks</a>
      <a href="update_status.php"     class="ep-action-btn ep-btn-secondary">🦸 My accepted tasks</a>
    ...
    <div class="ep-recent-card">
      <div class="ep-section-header">
        ...
        <a href="update_status.php" class="ep-view-all">View all →</a>
      </div>

      <?php if (mysqli_num_rows($recent_result) === 0): ?>
        <div class="ep-empty">No tasks yet — start browsing errands! 🌸</div>
      <?php else: ?>
